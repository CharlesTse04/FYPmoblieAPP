        ...
        <>
            ...
            <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                    <Pie
                        data={data}
                        dataKey="qty" // Key for quantities
                        nameKey="name" // Key for gallery names
                        label={(entry) => `${entry.name} (${entry.id}): ${entry.qty}`} // Custom label to show name and ID
                    >
                        {data.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={colors[index]} />
                        ))}
                    </Pie>
                    <Tooltip formatter={(value, name, props) => [`${value}`, `${props.payload.name} (ID: ${props.payload.id})`]} />
                    <Legend />
                </PieChart>
            </ResponsiveContainer>
            <h1>Gallery List</h1>
