...
import { View, Text, StyleSheet, FlatList, ActivityIndicator, RefreshControl, Alert, Button } from 'react-native';
import { getDatabase, ref, onValue, get, set,off } from 'firebase/database';
...
const CompetitionScreen = () => {
  ...
  const handleJoinCompetition = async (competitionId: string) => {
    try {
      ...
      // 檢查比賽是否已結束
      const competitionRef = ref(db, `competitions/${competitionId}`);
      const competitionSnapshot = await get(competitionRef);
      const competitionData = competitionSnapshot.val();

      const contestantCount = competitionData.contestant ? Object.keys(competitionData.contestant).length : 0;
      setContestantCount(contestantCount);
      if (contestantCount >= competitionData.participants) {
        Alert.alert('報名人數已滿');
        return;
      }

      // 寫入用戶資料
      await set(ref(db, `competitions/${competitionId}/contestant/${userId}`), {
        fraction: 0,
        outgoing: false,
        seat: 0,
        state: "join"
      ...
      Alert.alert('加入失敗', error instanceof Error ? error.message : '未知錯誤');
    }
  };
  ...
  const fetchCompetitions = async () => {
    ...
    try {
      ...
      for (const key of Object.keys(data)) {
        ...
        if (!contestantSnapshot.exists() && new Date(data[key].endDate) > new Date()) {
  ...
  const renderItem = ({ item }: { item: Competition }) => (
    <View style={styles.card}>
      ...
      <View style={styles.detailRow}>
        ...
        <Text style={styles.value}>{contestantCount}/{item.participants}</Text>
      </View>
      <View style={styles.detailRow}>
...
function unsubscribe() {
    const db = getDatabase(cong);
    const competitionsRef = ref(db, 'competitions');
    off(competitionsRef); // Detach all listeners from the 'competitions' reference
}
