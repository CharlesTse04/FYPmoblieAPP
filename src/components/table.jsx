...
const OrderTable = () => {
  ...
  const handleSearch = () => {
    const newFilteredRows = rows.filter((row) => {
      ...
      // Check if the status includes "未發貨"
      const isNotShipped = row.status.toLowerCase().includes("未發貨");
      ...
      return (
        ...
        isWithinDateRange &&
        isNotShipped // Include the check for "未發貨"
      );
    });
  ...
  const handleSave = async () => {
    ...
    const versionNumRef = await get(ref(db, `/order/${orderId}/${vision}/`));
    const versionRef = ref(db, `/order/${orderId}/${newVersion}/`);
    await set(versionRef, {
      ...editRow,
      date: new Date().toISOString(),
      price: versionNumRef.val().price,
      card: cardDetails,
      employee: LoginId().id,
      status: "complete",
    });
